<template>
  <el-dialog title="详情" :visible.sync="dialogVisible" width="70%" :before-close="handleClose">
    <el-tabs v-model="activeTabId" @tab-click="handleChangeTab">
      <el-tab-pane class="tab-pane" v-for="item in tabList" :label="item.name" :key="item.id" :name="String(item.id)">
        <p class="title">{{ detailInfo.name }}（{{ detailInfo.cnName }}）</p>
        <p class="des">{{ detailInfo.remark }}</p>
        <template>
          <p class="item-title">请求参数</p>
          <el-table :data="detailInfo.inParamList" row-key="id" border default-expand-all :tree-props="{ children: 'children' }">
            ...
            <el-table-column prop="exampleValue" label="示例值"> </el-table-column>
            <el-table-column prop="remark" label="描述"> </el-table-column>
          </el-table>
        </template>
        <template>
          <p class="item-title">响应参数</p>
          <el-table :data="detailInfo.outParamList" row-key="id" border default-expand-all :tree-props="{ children: 'children' }">
            <el-table-column prop="name" label="名称"> </el-table-column>
            <el-table-column prop="type" label="类型"> </el-table-column>
            <el-table-column prop="exampleValue" label="示例值"> </el-table-column>
            <el-table-column prop="remark" label="描述"> </el-table-column>
        ...
        <template>
          ...
          <el-table :data="detailInfo.errorCodeList" row-key="id" border>
            <el-table-column prop="errorCode" label="错误码" width="200"> </el-table-column>
            <el-table-column prop="errorMsg" label="错误信息"> </el-table-column>
            <el-table-column prop="solution" label="处理建议"> </el-table-column>
          </el-table>
        </template>
        <template>
          <p class="item-title">JSF方法配置</p>
          <div class="config-content">
            <p class="config-item">异常类型白名单：{{ configInfo.whiteExcps }}</p>
            <p class="config-item">异常类型白名单生效范围：{{ configInfo.effRange }}</p>
          </div>
        </template>
      </el-tab-pane>
    </el-tabs>
...
export default {
  ...
  computed: {
    configInfo() {
      ...
      const whiteExcps = (configObj.respRules && Object.keys(configObj.respRules.whiteExcps || {}).join(",")) || "";
      let effRange = "";
      const effRangeList = (configObj.effRange && Object.keys(configObj.effRange)) || [];
  ...
  methods: {
    queryPendingMethodList() {
      ...
      lopOpenApidepot.PublicApiPublishAudit.queryPendingMethodList(params)
        .then(response => {
    ...
    handleChangeTab() {
      ...
      lopOpenApidepot.PublicApiPublishAudit.queryPendingMethodApiDoc(params)
        .then(response => {
