    ...
    >
      ...
      >
        <el-form-item :prop="prop" :label="label">
          <template v-if="prop === 'envSite'">
            <el-select
              v-model="searchFormData[prop]"
              :placeholder="placeholder"
              @change="changeEnvSite"
              style="width: 100%"
            >
              <el-option
          ...
          <template v-else-if="prop === 'appKey'">
            <el-input
              v-model="searchFormData[prop]"
              :placeholder="placeholder"
              :clearable="true"
          ...
          <template v-else-if="prop === 'dockingScheme'">
            ...
            >
              <el-option
                v-for="{ code, name, hasUat } in dockingSchemeOptionList"
                :key="code"
                :value="{ code, name, hasUat }"
                :label="name"
              ></el-option>
            </el-select>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      searchFormList: [
        { label: "运行环境", prop: "envSite", placeholder: "请选择运行环境" },
      ...
      searchFormRules: {
        envSite: [{ required: true, message: "请输入运行环境" }],
      ...
      envOptionList: [
        { value: 1, label: "美国生产环境" },
        { value: 2, label: "美国预发环境" },
        { value: 3, label: "荷兰生产环境" },
        { value: 4, label: "荷兰预发环境" },
      ],
      statusOptionList: [
  ...
  methods: {
    ...
    async queryApprovedRecordList() {
      const { envSite, appKey } = this.searchFormData;
      const params = { envSite, appKey };
      return await lcp.i18n.rest.bizOrder.queryApprovedRecordList(params);
    },
    ...
    async queryOuterBizOrderApiList() {
      ...
      const params = { envSite, appKey, unitCode: code };
      return await lcp.i18n.rest.bizApi.queryOuterBizOrderApiList(params);
    },
    // 查询日志表格数据
    async queryAppLogsList() {
      ...
      return await lcp.i18n.rest.loggingTool.queryAppLogsList(data);
    },
    ...
    // 输入AppKey时，重置对接方案、API的URI 获取对应对接方案选项列表
    onInputAppKey(value) {
      if (this.searchFormData.appKey && this.searchFormData.envSite) {
        this.resetOptionList();
      }
    },

    // 重置对接方案、API的URI 获取对应对接方案选项列表
    resetOptionList() {
