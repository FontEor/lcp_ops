  ...
  <div class="apiApplyAmount">
    <header class="header">
      ...
      <div class="select-wrapper">
        <span class="select-label">开发商</span>
        <el-select
          v-model="supplierId"
          placeholder="请选择开发商"
          @change="requestChartData"
  ...
  methods: {
    ...
    initSupplierOptions() {
      (async () => {
        ...
        const { data } = response;
        const supplierOptionsData = [{ label: "全部", value: 0 }];
        data &&
          data.length > 0 &&
          data.forEach((item) => {
            const label = item.nameAndCode;
            const value = item.id;
            supplierOptionsData.push({ label, value });
          });
        this.supplierOptions = supplierOptionsData;
      })();
    },
    requestChartData() {
      ...
      const data = [{ startTime, endTime, supplierId }];
      (async () => {
        const response = await api.report
          .findApplyTotal(data)
          .catch((error) => {
            ...
            throw new Error("requestChartData findApplyTotal");
          });
      ...
      (async () => {
        const response = await api.report
          .findApplyNewAndAverage(data)
          .catch((error) => {
            const { message } = error;
            message && this.$message.error(message);
            throw new Error("requestChartData findApplyNewAndAverage");
          });
        const { legend, category, series } = response.data;
